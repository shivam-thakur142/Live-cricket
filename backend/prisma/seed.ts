...
async function main() {
  console.log("[seed] Wiping existing rows (data only, schema untouched)...");
  await prisma.$transaction([
    prisma.commentaryEvent.deleteMany(),
    prisma.ballEvent.deleteMany(),
    prisma.battingCardEntry.deleteMany(),
    prisma.bowlingCardEntry.deleteMany(),
  ...
  const [adminHash, editorHash, viewerHash] = await Promise.all([
    bcrypt.hash(env.SEED_ADMIN_PASSWORD, 10),
    bcrypt.hash(env.SEED_EDITOR_PASSWORD, 10),
    bcrypt.hash(env.SEED_VIEWER_PASSWORD, 10),
  ]);
  await prisma.user.createMany({
    data: [
      { id: "u1", name: "Rahul Verma", email: env.SEED_ADMIN_EMAIL, passwordHash: adminHash, role: "admin" },
      { id: "u2", name: "Anita Rana", email: env.SEED_EDITOR_EMAIL, passwordHash: editorHash, role: "editor" },
      { id: "u3", name: "Vikas Bhatt", email: env.SEED_VIEWER_EMAIL, passwordHash: viewerHash, role: "viewer" },
    ],
  });
  console.log(`[seed] Users: admin=${env.SEED_ADMIN_EMAIL} editor=${env.SEED_EDITOR_EMAIL} viewer=${env.SEED_VIEWER_EMAIL}`);
  ...
  for (const team of TEAMS) {
    await prisma.player.createMany({
      data: SQUADS[team.id].map((name, idx) => {
        const role = ROLE_DISTRIBUTION[idx];
        return {
          id: `${team.prefix}-${idx + 1}`,
          name,
          teamId: team.id,
          role,
          jerseyNumber: idx + 1,
  ...
  await prisma.tournamentTeam.createMany({
    data: TEAMS.map((t) => ({ id: `tt-scl-${t.prefix}`, tournamentId: "scl-2026", teamId: t.id })),
  });
  ...
  await prisma.match.createMany({
    data: [
      { id: "scl-m7", tournamentId: "scl-2026", matchNumber: 7, stage: "League", date: new Date("2026-01-21"), time: "10:00", venueId: "dharampur-stadium", team1Id: "kala-amb-kings", team2Id: "sirmour-super-kings", status: "upcoming" },
      { id: "scl-m8", tournamentId: "scl-2026", matchNumber: 8, stage: "League", date: new Date("2026-01-21"), time: "14:00", venueId: "paonta-sports-ground", team1Id: "nahan-warriors", team2Id: "paonta-strikers", status: "upcoming" },
      { id: "ntc-m1", tournamentId: "nahan-t10-2026", matchNumber: 1, stage: "League", date: new Date("2026-03-05"), time: "18:00", venueId: "dharampur-stadium", team1Id: "kala-amb-kings", team2Id: "nahan-warriors", status: "upcoming" },
      { id: "pcc-m1", tournamentId: "paonta-championship-2026", matchNumber: 1, stage: "League", date: new Date("2026-04-10"), time: "09:30", venueId: "paonta-sports-ground", team1Id: "paonta-strikers", team2Id: "rajgarh-royals", status: "upcoming" },
    ],
  });
